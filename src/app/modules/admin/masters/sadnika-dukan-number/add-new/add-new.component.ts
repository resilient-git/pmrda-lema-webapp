...
import { AddNewService } from 'app/modules/admin/masters/sadnika-dukan-number/add-new/add-new.service';
...
export class AddNewComponent implements OnInit, OnDestroy
{
    ...
    ngOnInit(): void
    {
        this.sadnikaForm = this._formBuilder.group({
            ...
            type_of_property:['',Validators.required],
    ...
    getSocietyList(code):void{
        this.societyData = [];
        console.log(this.sadnikaForm.value.scheme_number);
        
        this._masterService.getSocietyData(code, this.sadnikaForm.value['scheme_number'])
            .subscribe(() => {
                if (this._masterService.data$.source['_value'].status != 200) {
                    // Set the alert
                    this.alert = {
     ...
     */
     send(): void
     {
        ...
        // Sign up
        this._addnewService.postData(this.sadnikaForm.value)
            .subscribe((response) => {
