...
import { CalculationService } from 'app/modules/desk3/property/application/transfer/calculation/calculation.service';
...
import { Desk3CalculationTransferModule } from "./calculation.module";
...
export class CalculationComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<Desk3CalculationTransferModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<Desk3CalculationTransferModule>;
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._calculationService.getCalculationDesk3()
      .subscribe(() => {
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__calculationService.approveDesk3(this.approveForm.value)
        .subscribe((response) => {
...
sendRejection(): void {
  ...
  // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__calculationService.rejectDesk3(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
