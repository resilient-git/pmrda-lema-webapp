...
import { DeedVerificationService } from 'app/modules/desk3/property/application/transfer/deed_verification/deed_verification.service';
...
import { Desk3DeedVerificationTransferModule } from "./deed_verification.module";
...
export class DeedVerificationComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<Desk3DeedVerificationTransferModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<Desk3DeedVerificationTransferModule>;
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._deed_verificationService.getDeedVerificationDesk3()
      .subscribe(() => {
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__deed_verificationService.approveDesk3(this.approveForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
               ...
               this._router.navigateByUrl('/desk3/deed_verification-transfer-applications');
...
sendRejection(): void {
  ...
  // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__deed_verificationService.rejectDesk3(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
