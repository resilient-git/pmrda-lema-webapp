...
import { CertificateService } from 'app/modules/financial-controller/property/application/heirship/certificate/certificate.service';
...
import { FinancialControllerCertificateHeirshipModule } from "./certificate.module";
...
export class CertificateComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<FinancialControllerCertificateHeirshipModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<FinancialControllerCertificateHeirshipModule>;
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._certificateService.getCertificateFinancialController()
      .subscribe(() => {
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyHeirshipForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__certificateService.approveFinancialController(this.approveForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
               ...
               this._router.navigateByUrl('/financial-controller/certificate-heirship-applications');
...
sendRejection(): void {
  ...
  // this.propertyHeirshipForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__certificateService.rejectFinancialController(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
