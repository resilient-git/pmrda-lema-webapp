...
import { DeedVerificationService } from 'app/modules/financial-controller/property/application/transfer/deed_verification/deed_verification.service';
...
import { FinancialControllerDeedVerificationTransferModule } from "./deed_verification.module";
...
@Component({
  selector: 'deed_verification',
  templateUrl: './deed_verification.component.html',
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class DeedVerificationComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<FinancialControllerDeedVerificationTransferModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<FinancialControllerDeedVerificationTransferModule>;
  ...
  constructor(
    ...
    private __deed_verificationService:DeedVerificationService,
    private _deed_verificationService: DeedVerificationService,
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._deed_verificationService.getDeedVerificationFinancialController()
      .subscribe(() => {
        if (this._deed_verificationService.data$.source['_value'].status != 200) {
          // Set the alert
          this.alert = {
            type: 'error',
            message: this._deed_verificationService.data$.source['_value'].response
          };

          // Show the alert
          this.showAlert = true;
        } else {
          this.dataSource.data = this._deed_verificationService.data$.source['_value'].data;
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__deed_verificationService.approveFinancialController(this.approveForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
               ...
               this._router.navigateByUrl('/financial-controller/deed_verification-transfer-applications');
...
sendRejection(): void {
  ...
  // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__deed_verificationService.rejectFinancialController(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
    ...
    */  
    ...
    //     this._deed_verificationService.deleteProperty({ code, user_id })
    //       .subscribe((response) => {
    //         if (response.status != 200) {
    //           // Set the alert
    //           this.alert = {
