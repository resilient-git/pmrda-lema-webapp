...
import { CertificateService } from 'app/modules/joint-commissioner/property/application/heirship/certificate/certificate.service';
...
import { JointCommissionerCertificateHeirshipModule } from "./certificate.module";
...
export class CertificateComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<JointCommissionerCertificateHeirshipModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<JointCommissionerCertificateHeirshipModule>;
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._certificateService.getCertificateJointCommissioner()
      .subscribe(() => {
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyHeirshipForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__certificateService.approveJointCommissioner(this.approveForm.value)
        .subscribe((response) => {
...
sendRejection(): void {
  ...
  // this.propertyHeirshipForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__certificateService.rejectJointCommissioner(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
