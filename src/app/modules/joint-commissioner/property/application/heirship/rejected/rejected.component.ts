...
import { RejectedService } from 'app/modules/joint-commissioner/property/application/transfer/rejected/rejected.service';
...
import { JointCommissionerRejectedTransferModule } from "./rejected.module";
...
export class RejectedComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<JointCommissionerRejectedTransferModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<JointCommissionerRejectedTransferModule>;
  ...
  ngOnInit(): void {
   ...
   let user = cloneDeep(JSON.parse(localStorage.getItem('currentUser')));
    ...
    const code = user.id;
    this._rejectedService.getRejectedJointCommissioner()
      .subscribe(() => {
