...
import { JointCommissionerDeedVerificationTransferModule } from "./deed_verification.module";
...
export class DeedVerificationComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<JointCommissionerDeedVerificationTransferModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<JointCommissionerDeedVerificationTransferModule>;
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._deed_verificationService.getDeedVerificationJointCommissioner()
      .subscribe(() => {
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__deed_verificationService.approveJointCommissioner(this.approveForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
               ...
               this._router.navigateByUrl('/joint-commissioner/deed_verification-transfer-applications');
...
sendRejection(): void {
  ...
  // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__deed_verificationService.rejectJointCommissioner(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
