...
import { CalculationService } from 'app/modules/joint-commissioner/property/application/transfer/calculation/calculation.service';
...
import { JointCommissionerCalculationTransferModule } from "./calculation.module";
...
export class CalculationComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<JointCommissionerCalculationTransferModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<JointCommissionerCalculationTransferModule>;
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._calculationService.getCalculationJointCommissioner()
      .subscribe(() => {
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__calculationService.approveJointCommissioner(this.approveForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
               ...
               this._router.navigateByUrl('/joint-commissioner/calculation-transfer-applications');
...
sendRejection(): void {
  ...
  // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__calculationService.rejectJointCommissioner(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
