...
import { JointCommissionerVerificationBankNocTransferModule } from "./verification.module";
...
export class JointCommissionerVerificationBankNocComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<JointCommissionerVerificationBankNocTransferModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<JointCommissionerVerificationBankNocTransferModule>;
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._verificationBankNocService.getVerificationJointCommissioner()
      .subscribe(() => {
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__verificationBankNocService.approveJointCommissioner(this.approveForm.value)
        .subscribe((response) => {
...
sendRejection(): void {
  ...
  // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__verificationBankNocService.rejectJointCommissioner(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
