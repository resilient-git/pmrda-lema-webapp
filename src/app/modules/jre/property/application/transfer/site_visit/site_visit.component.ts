...
import { JreSiteVisitTransferModule } from "./site_visit.module";
...
@Component({
  selector: 'site_visit',
  templateUrl: './site_visit.component.html',
...
export class SiteVisitComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<JreSiteVisitTransferModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<JreSiteVisitTransferModule>;
   ...
   */
  constructor(
    private _siteVisitService: SiteVisitService,
  ...
  ngOnInit(): void {
   ...
   let user = cloneDeep(JSON.parse(localStorage.getItem('currentUser')));
    ...
    const code = user.id;
    this._siteVisitService.getSiteVisitJre()
      .subscribe(() => {
        if (this._siteVisitService.data$.source['_value'].status != 200) {
          // Set the alert
          this.alert = {
            type: 'error',
            message: this._siteVisitService.data$.source['_value'].response
          };

          // Show the alert
          this.showAlert = true;
        } else {
          this.dataSource.data = this._siteVisitService.data$.source['_value'].data;
