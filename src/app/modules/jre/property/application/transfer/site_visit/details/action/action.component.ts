...
import { ViewPropertyTransferActionService } from './action.service';
    ...
    onSelect(event) {
      ...
      if (event.target.files) {
            ...
            reader.onload = (event: any) => {
              this.sthalpaniAhvalData = btoa(event.target.result);
            };             
          }
    }
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this._viewPropertyTransferActionService.approveJre(this.approveForm.value,this.sthalpaniAhvalData)
        .subscribe((response) => {
            if (response.status != 200) {
               ...
               this._router.navigateByUrl('/jre/site-visit-report');
