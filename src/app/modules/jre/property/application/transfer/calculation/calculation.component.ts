...
import { JreCalculationTransferModule } from "./calculation.module";
...
export class CalculationComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<JreCalculationTransferModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<JreCalculationTransferModule>;
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._calculationService.getCalculationJre()
      .subscribe(() => {
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__calculationService.approveJre(this.approveForm.value)
        .subscribe((response) => {
...
sendRejection(): void {
  ...
  // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__calculationService.rejectJre(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
