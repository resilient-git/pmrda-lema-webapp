...
import { TahsildarVerificationBankNocTransferModule } from "./verification.module";
...
export class VerificationBankNocComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<TahsildarVerificationBankNocTransferModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<TahsildarVerificationBankNocTransferModule>;
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._verificationBankNocService.getVerificationTahsildar()
      .subscribe(() => {
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__verificationBankNocService.approveTahsildar(this.approveForm.value)
        .subscribe((response) => {
...
sendRejection(): void {
  ...
  // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__verificationBankNocService.rejectTahsildar(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
