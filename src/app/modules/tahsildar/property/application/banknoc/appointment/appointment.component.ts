...
import { AppointmentService } from 'app/modules/tahsildar/property/application/banknoc/transfer/appointment/appointment.service';
...
import { TahsildarAppointmentTransferModule } from "./appointment.module";
...
export class AppointmentComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<TahsildarAppointmentTransferModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<TahsildarAppointmentTransferModule>;
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._appointmentService.getAppointmentTahsildar()
      .subscribe(() => {
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__appointmentService.approveTahsildar(this.approveForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
               ...
               this._router.navigateByUrl('/tahsildar/appointment-transfer-applications');
...
sendRejection(): void {
  ...
  // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__appointmentService.rejectTahsildar(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
