...
import { DeedVerificationService } from 'app/modules/tahsildar/property/application/transfer/deed_verification/deed_verification.service';
...
import { TahsildarDeedVerificationTransferModule } from "./deed_verification.module";
...
export class DeedVerificationComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<TahsildarDeedVerificationTransferModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<TahsildarDeedVerificationTransferModule>;
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._deed_verificationService.getDeedVerificationTahsildar()
      .subscribe(() => {
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__deed_verificationService.approveTahsildar(this.approveForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
               ...
               this._router.navigateByUrl('/tahsildar/deed_verification-transfer-applications');
...
sendRejection(): void {
  ...
  // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__deed_verificationService.rejectTahsildar(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
