...
import { ViewPropertyTransferActionService } from 'app/modules/desk1/property/application/transfer/genrate_deed/details/action/action.service';
import { MastersService } from 'app/modules/admin/masters/masters.service';
import { FuseAlertType } from '@fuse/components/alert';
import { MatDialog } from '@angular/material/dialog';
import { MatTable, MatTableDataSource } from "@angular/material/table";
import{ ViewPropertyTransferActionModule } from 'app/modules/desk1/property/application/transfer/genrate_deed/details/action/action.module'
@Component({
        ...
        this.approveForm = this._formBuilder.group({
          //  sales_amount:['', Validators.required],
           // market_value:['', Validators.required],
            towards_east:['', Validators.required],
            towards_west:['', Validators.required],
            towards_north:['', Validators.required],
            towards_south:['', Validators.required],
            created_by:[],
            created_at:[] 
          });
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this._viewPropertyTransferActionService.approveDesk1DeedGenrate(this.approveForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
               ...
               this._router.navigateByUrl('/clerk/transfer-applications/deed-generation-pending');
