...
import { Desk1DeedGenrateTransferModule } from "./genrate_deed.module";
...
@Component({
  selector: 'genrate_deed',
  templateUrl: './genrate_deed.component.html',
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class DeedGenrateComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<Desk1DeedGenrateTransferModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<Desk1DeedGenrateTransferModule>;
  ...
  constructor(
    ...
    private __genrate_deedService:DeedGenrateService,
    private _genrate_deedService: DeedGenrateService,
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._genrate_deedService.getDeedGenrateDesk1()
      .subscribe(() => {
        if (this._genrate_deedService.data$.source['_value'].status != 200) {
          // Set the alert
          this.alert = {
            type: 'error',
            message: this._genrate_deedService.data$.source['_value'].response
          };

          // Show the alert
          this.showAlert = true;
        } else {
          this.dataSource.data = this._genrate_deedService.data$.source['_value'].data;
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__genrate_deedService.approveDesk1(this.approveForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
               ...
               this._router.navigateByUrl('/clerk/genrate_deed-transfer-applications');
...
sendRejection(): void {
  ...
  // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__genrate_deedService.rejectDesk1(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
              ...
              this._router.navigateByUrl('/desk1/genrate_deed-transfer-applications');
    ...
    */  
    ...
    //     this._genrate_deedService.deleteProperty({ code, user_id })
    //       .subscribe((response) => {
    //         if (response.status != 200) {
    //           // Set the alert
    //           this.alert = {
