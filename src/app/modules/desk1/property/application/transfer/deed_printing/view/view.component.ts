...
export class ViewComponent implements OnInit
{
    ...
    onSelect(event,fileFor) {
      ...
      if (event.target.files) {
            ...
            reader.onload = (event: any) => {
              ...
              this.deedSignedLastPageData = btoa(event.target.result);
            };             
          }
  }
    //   OTP verificatio
    onOtpChange(value: string[]): void {
      ...
      ngOnInit(): void {
        ...
        this.approveForm = this._formBuilder.group({
          deed_signed_last_page : ['', Validators.required],
          created_by:[],
          created_at:[] 
        });
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this._viewApplicationService.deedHandover(this.approveForm.value,this.deedSignedLastPageData)
        .subscribe((response) => {
            if (response.status != 200) {
               ...
               this._router.navigateByUrl('/desk1/transfer-applications/deed-printing-pending');
