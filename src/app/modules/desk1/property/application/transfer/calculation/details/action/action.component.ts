...
import { ViewPropertyTransferActionService } from 'app/modules/desk1/property/application/transfer/calculation/details/action/action.service';
import { MastersService } from 'app/modules/admin/masters/masters.service';
import { FuseAlertType } from '@fuse/components/alert';
import { MatDialog } from '@angular/material/dialog';
import { MatTable, MatTableDataSource } from "@angular/material/table";
import{ ViewPropertyTransferActionModule } from 'app/modules/desk1/property/application/transfer/calculation/details/action/action.module'
@Component({
...
export class ViewPropertyTransferActionComponent implements OnInit
{
    ...
    ngOnInit(): void
    {
        ...
        this.approveForm = this._formBuilder.group({
            ...
            form_1_attached : ['', Validators.required],
            loan_details : ['', Validators.required],
            society_details : ['', Validators.required],
            society_noc_details : ['', Validators.required],
            mou_details : ['', Validators.required],
            additional_work_letter : ['', Validators.required],
            basic_premium_amount : ['', Validators.required],
            form_1_amount : ['', Validators.required],
            transfer_fee : [''],
            additional_work_details : ['', Validators.required],            
        ...
        .subscribe(() => {
            if (this._masterService.data$.source['_value'].status != 200) {
                ...
                this.approveForm.value['transfer_fee'] = this.pta.transfer_fee;
                //console.log(this.pta.transfer_fee);
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this._viewPropertyTransferActionService.approveTransferFeeDesk1(this.approveForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
               ...
               this._router.navigateByUrl('/clerk/transfer-applications/calculation-pending');
...
sendRejection(): void {
  ...
  // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this._viewPropertyTransferActionService.rejectDesk1(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
              ...
              this._router.navigateByUrl('/desk1/calculation-transfer-applications');
