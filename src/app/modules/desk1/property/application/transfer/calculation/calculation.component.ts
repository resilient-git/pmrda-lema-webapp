...
import { CalculationService } from 'app/modules/desk1/property/application/transfer/calculation/calculation.service';
...
import { Desk1CalculationTransferModule } from "./calculation.module";
...
export class CalculationComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<Desk1CalculationTransferModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<Desk1CalculationTransferModule>;
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._calculationService.getCalculationDesk1()
      .subscribe(() => {
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__calculationService.approveDesk1(this.approveForm.value)
        .subscribe((response) => {
...
sendRejection(): void {
  ...
  // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__calculationService.rejectDesk1(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
