...
import { Desk1DeedHandoverTransferModule } from "./deed_handover.module";
...
@Component({
  selector: 'deed_handover',
  templateUrl: './deed_handover.component.html',
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class DeedHandoverComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<Desk1DeedHandoverTransferModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<Desk1DeedHandoverTransferModule>;
  ...
  constructor(
    ...
    private __deed_handoverService:DeedHandoverService,
    private _deed_handoverService: DeedHandoverService,
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._deed_handoverService.getDeedHandoverDesk1()
      .subscribe(() => {
        if (this._deed_handoverService.data$.source['_value'].status != 200) {
          // Set the alert
          this.alert = {
            type: 'error',
            message: this._deed_handoverService.data$.source['_value'].response
          };

          // Show the alert
          this.showAlert = true;
        } else {
          this.dataSource.data = this._deed_handoverService.data$.source['_value'].data;
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__deed_handoverService.approveDesk1(this.approveForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
               ...
               this._router.navigateByUrl('/desk1/deed_handover-transfer-applications');
...
sendRejection(): void {
  ...
  // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__deed_handoverService.rejectDesk1(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
    ...
    */  
    ...
    //     this._deed_handoverService.deleteProperty({ code, user_id })
    //       .subscribe((response) => {
    //         if (response.status != 200) {
    //           // Set the alert
    //           this.alert = {
