...
import { ViewPropertyTransferActionService } from 'app/modules/desk1/property/application/transfer/file_close/details/action/action.service';
import { MastersService } from 'app/modules/admin/masters/masters.service';
import { FuseAlertType } from '@fuse/components/alert';
import { MatDialog } from '@angular/material/dialog';
import { MatTable, MatTableDataSource } from "@angular/material/table";
import{ ViewPropertyTransferActionModule } from 'app/modules/desk1/property/application/transfer/file_close/details/action/action.module'
@Component({
...
export class ViewPropertyTransferActionComponent implements OnInit
{
    ...
    ngOnInit(): void
    {
        ...
        this.approveForm = this._formBuilder.group({
          signed_certificate : ['', Validators.required],
            created_by:[],
            created_at:[] 
          });
    ...
    onSelect(event) {
      ...
      if (event.target.files) {
            ...
            reader.onload = (event: any) => {
              this.signedCertificateData = btoa(event.target.result);
            };             
          }
    }
    dummyBinding():void{
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this._viewPropertyTransferActionService.approveDesk1(this.approveForm.value,this.signedCertificateData)
        .subscribe((response) => {
            if (response.status != 200) {
               ...
               this._router.navigateByUrl('/clerk/transfer-applications/file-close-pending');
    ...
    this._viewPropertyTransferActionService.rejectDesk1(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
              ...
              this._router.navigateByUrl('/clerk/file_close-transfer-applications');
