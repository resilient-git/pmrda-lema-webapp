...
import { DeedVerificationService } from 'app/modules/desk1/property/application/transfer/deed_verification/deed_verification.service';
...
import { Desk1DeedVerificationTransferModule } from "./deed_verification.module";
...
export class DeedVerificationComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<Desk1DeedVerificationTransferModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<Desk1DeedVerificationTransferModule>;
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._deed_verificationService.getDeedVerificationDesk1()
      .subscribe(() => {
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__deed_verificationService.approveDesk1(this.approveForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
               ...
               this._router.navigateByUrl('/clerk/deed_verification-transfer-applications');
...
sendRejection(): void {
  ...
  // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__deed_verificationService.rejectDesk1(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
