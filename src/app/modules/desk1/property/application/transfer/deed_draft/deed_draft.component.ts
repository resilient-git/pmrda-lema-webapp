...
import { DeedDraftService } from 'app/modules/desk1/property/application/transfer/deed_draft/deed_draft.service';
...
import { Desk1DeedDraftTransferModule } from "./deed_draft.module";
...
@Component({
  selector: 'deed_draft',
  templateUrl: './deed_draft.component.html',
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class DeedDraftComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<Desk1DeedDraftTransferModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<Desk1DeedDraftTransferModule>;
  ...
  constructor(
    ...
    private __deed_draftService:DeedDraftService,
    private _deed_draftService: DeedDraftService,
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._deed_draftService.getDeedDraftDesk1()
      .subscribe(() => {
        if (this._deed_draftService.data$.source['_value'].status != 200) {
          // Set the alert
          this.alert = {
            type: 'error',
            message: this._deed_draftService.data$.source['_value'].response
          };

          // Show the alert
          this.showAlert = true;
        } else {
          this.dataSource.data = this._deed_draftService.data$.source['_value'].data;
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__deed_draftService.approveDesk1(this.approveForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
               ...
               this._router.navigateByUrl('/desk1/deed_draft-transfer-applications');
...
sendRejection(): void {
  ...
  // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__deed_draftService.rejectDesk1(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
    ...
    */  
    ...
    //     this._deed_draftService.deleteProperty({ code, user_id })
    //       .subscribe((response) => {
    //         if (response.status != 200) {
    //           // Set the alert
    //           this.alert = {
