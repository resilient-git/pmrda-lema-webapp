...
import { ViewPropertyTransferVerifyService } from 'app/modules/desk1/property/application/transfer/verification_close/details/verify/verify.service';
import { MastersService } from 'app/modules/admin/masters/masters.service';
import { FuseAlertType } from '@fuse/components/alert';
import { MatDialog } from '@angular/material/dialog';
import { MatTable, MatTableDataSource } from "@angular/material/table";
import{ ViewPropertyTransferVerifyConfirmModule } from 'app/modules/desk1/property/application/transfer/verification_close/details/verify/verify.module'
import {WebcamImage, WebcamInitError, WebcamUtil} from 'ngx-webcam';

@Component({
    selector       : 'ViewPropertyTransferVerify',
...
export class ViewPropertyTransferVerifyConfirmComponent implements OnInit
{
    ...
    pta : any;
    dataSource: MatTableDataSource<ViewPropertyTransferVerifyConfirmModule> = new MatTableDataSource([]);
    displayedColumns: string[] = ['srno','purchaser_name','verifys'];
     ...
     */
    constructor(
        private _viewPropertyTransferVerifyService: ViewPropertyTransferVerifyService,
    ...
    public triggerSnapshot(tid): void {
      this.currentImgId = tid;
      this.trigger.next();
    }
    ...
    public handleImage(webcamImage: WebcamImage): void {
      ...
      this.capturedImg[this.currentImgId]=webcamImage;
  ...
  */
  ...
  Object.keys(this.capturedImg).forEach(key => {
    console.log(key, this.capturedImg[key]);
    headshots.push({"idx":key,"imgData":btoa(this.capturedImg[key].imageAsDataUrl)})
  });

    //[this.currentImgId]=webcamImage;
    this.approveForm.value['headshots'] = headshots;
//btoa
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this._viewPropertyTransferVerifyService.approveDesk1(this.approveForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
               ...
               this._router.navigateByUrl('/desk1/transfer-applications/verification_close-pending');
