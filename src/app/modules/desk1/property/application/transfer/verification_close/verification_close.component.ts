...
import { VerificationCloseService } from 'app/modules/desk1/property/application/transfer/verification_close/verification_close.service';
...
import { Desk1VerificationCloseTransferModule } from "./verification_close.module";
...
@Component({
  selector: 'verification_close',
  templateUrl: './verification_close.component.html',
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class VerificationCloseComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<Desk1VerificationCloseTransferModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<Desk1VerificationCloseTransferModule>;
  ...
  constructor(
    ...
    private __verification_closeService:VerificationCloseService,
    private _verification_closeService: VerificationCloseService,
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._verification_closeService.getVerificationCloseDesk1()
      .subscribe(() => {
        if (this._verification_closeService.data$.source['_value'].status != 200) {
          // Set the alert
          this.alert = {
            type: 'error',
            message: this._verification_closeService.data$.source['_value'].response
          };

          // Show the alert
          this.showAlert = true;
        } else {
          this.dataSource.data = this._verification_closeService.data$.source['_value'].data;
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__verification_closeService.approveDesk1(this.approveForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
               ...
               this._router.navigateByUrl('/desk1/verification_close-transfer-applications');
...
sendRejection(): void {
  ...
  // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__verification_closeService.rejectDesk1(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
    ...
    */  
    ...
    //     this._verification_closeService.deleteProperty({ code, user_id })
    //       .subscribe((response) => {
    //         if (response.status != 200) {
    //           // Set the alert
    //           this.alert = {
