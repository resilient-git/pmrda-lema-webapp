...
import { FileCloseService } from 'app/modules/desk1/property/application/heirship/file_close/file_close.service';
...
import { Desk1FileCloseHeirshipModule } from "./file_close.module";
...
@Component({
  selector: 'file_close',
  templateUrl: './file_close.component.html',
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class FileCloseComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<Desk1FileCloseHeirshipModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<Desk1FileCloseHeirshipModule>;
  ...
  constructor(
    ...
    private __file_closeService:FileCloseService,
    private _file_closeService: FileCloseService,
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._file_closeService.getFileCloseDesk1()
      .subscribe(() => {
        if (this._file_closeService.data$.source['_value'].status != 200) {
          // Set the alert
          this.alert = {
            type: 'error',
            message: this._file_closeService.data$.source['_value'].response
          };

          // Show the alert
          this.showAlert = true;
        } else {
          this.dataSource.data = this._file_closeService.data$.source['_value'].data;
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyHeirshipForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__file_closeService.approveDesk1(this.approveForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
               ...
               this._router.navigateByUrl('/desk1/file_close-heirship-applications');
...
sendRejection(): void {
  ...
  // this.propertyHeirshipForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__file_closeService.rejectDesk1(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
    ...
    */  
    ...
    //     this._file_closeService.deleteProperty({ code, user_id })
    //       .subscribe((response) => {
    //         if (response.status != 200) {
    //           // Set the alert
    //           this.alert = {
