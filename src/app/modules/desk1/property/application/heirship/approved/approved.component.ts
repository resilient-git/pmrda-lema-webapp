...
import { Desk1ApprovedHeirshipTransferModule } from "./approved.module";
...
export class ApprovedHeirshipComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<Desk1ApprovedHeirshipTransferModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<Desk1ApprovedHeirshipTransferModule>;
   ...
   */
  constructor(
    private _approvedHeirshipService: ApprovedHeirshipService,
  ...
  ngOnInit(): void {
   ...
   let user = cloneDeep(JSON.parse(localStorage.getItem('currentUser')));
    ...
    const code = user.id;
    this._approvedHeirshipService.getApprovedDesk1()
      .subscribe(() => {
        if (this._approvedHeirshipService.data$.source['_value'].status != 200) {
          // Set the alert
          this.alert = {
            type: 'error',
            message: this._approvedHeirshipService.data$.source['_value'].response
          };

          // Show the alert
          this.showAlert = true;
        } else {
          this.dataSource.data = this._approvedHeirshipService.data$.source['_value'].data;
