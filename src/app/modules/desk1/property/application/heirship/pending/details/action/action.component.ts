...
import { ViewHeirshipTransferActionModule } from './action.module';
import { ViewHeirshipTransferActionService } from './action.service';
@Component({
    selector       : 'ViewHeirshipTransferAction',
...
export class ViewHeirshipTransferActionComponent implements OnInit
{
    ...
    pta : any;
    dataSource: MatTableDataSource<ViewHeirshipTransferActionModule> = new MatTableDataSource([]);
    displayedColumns: string[] = ['srno','purchaser_name','actions'];
    /**
     * Constructor
     */
    constructor(
        private _viewHeirshipTransferActionService: ViewHeirshipTransferActionService,
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this._viewHeirshipTransferActionService.approveDesk1(this.approveForm.value)
        .subscribe((response) => {
...
sendRejection(): void {
  ...
  // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this._viewHeirshipTransferActionService.rejectDesk1(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
