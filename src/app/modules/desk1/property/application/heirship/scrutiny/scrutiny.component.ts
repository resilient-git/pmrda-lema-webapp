...
import { ScrutinyService } from 'app/modules/desk1/property/application/heirship/scrutiny/scrutiny.service';
...
import { Desk1ScrutinyHeirshipModule } from "./scrutiny.module";
...
export class ScrutinyComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<Desk1ScrutinyHeirshipModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<Desk1ScrutinyHeirshipModule>;
  ...
  constructor(
    ...
    private __scrutinyService:ScrutinyService,
    private _scrutinyService: ScrutinyService,
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._scrutinyService.getScrutinyDesk1()
      .subscribe(() => {
        if (this._scrutinyService.data$.source['_value'].status != 200) {
          // Set the alert
          this.alert = {
            type: 'error',
            message: this._scrutinyService.data$.source['_value'].response
          };

          // Show the alert
          this.showAlert = true;
        } else {
          this.dataSource.data = this._scrutinyService.data$.source['_value'].data;
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyHeirshipForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__scrutinyService.approveDesk1(this.approveForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
               ...
               this._router.navigateByUrl('/desk1/scrutiny-heirship-applications');
...
sendRejection(): void {
  ...
  // this.propertyHeirshipForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__scrutinyService.rejectDesk1(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
    ...
    */  
    ...
    //     this._scrutinyService.deleteProperty({ code, user_id })
    //       .subscribe((response) => {
    //         if (response.status != 200) {
    //           // Set the alert
    //           this.alert = {
