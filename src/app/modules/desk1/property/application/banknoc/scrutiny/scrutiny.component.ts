...
import { Desk1BankNocScrutinyTransferModule} from "./scrutiny.module";
import { FuseConfirmationService } from "@fuse/services/confirmation";

import { cloneDeep } from 'lodash-es';
import { MastersService } from "app/modules/admin/masters/masters.service";
import { ScrutinyBankNocService } from "./scrutiny.service";
...
export class ScrutinyComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<Desk1BankNocScrutinyTransferModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<Desk1BankNocScrutinyTransferModule>;
  ...
  constructor(
    ...
    private __scrutinyBankNocService:ScrutinyBankNocService,
    private _scrutinyBankNocService: ScrutinyBankNocService,
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._scrutinyBankNocService.getScrutinyDesk1()
      .subscribe(() => {
        if (this._scrutinyBankNocService.data$.source['_value'].status != 200) {
          // Set the alert
          this.alert = {
            type: 'error',
            message: this._scrutinyBankNocService.data$.source['_value'].response
          };

          // Show the alert
          this.showAlert = true;
        } else {
          this.dataSource.data = this._scrutinyBankNocService.data$.source['_value'].data;
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__scrutinyBankNocService.approveDesk1(this.approveForm.value)
        .subscribe((response) => {
...
sendRejection(): void {
  ...
  // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__scrutinyBankNocService.rejectDesk1(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
