...
import { Desk1ApprovedBankNocTransferModule } from "./approved.module";
...
export class ApprovedBankNocComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<Desk1ApprovedBankNocTransferModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<Desk1ApprovedBankNocTransferModule>;
   ...
   */
  constructor(
    private _approvedService: ApprovedBankNocService,
  ...
  ngOnInit(): void {
   ...
   let user = cloneDeep(JSON.parse(localStorage.getItem('currentUser')));
    ...
    const code = user.id;
    this._approvedService.getApprovedDesk1()
      .subscribe(() => {
        if (this._approvedService.data$.source['_value'].status != 200) {
          // Set the alert
          this.alert = {
            type: 'error',
            message: this._approvedService.data$.source['_value'].response
          };

          // Show the alert
          this.showAlert = true;
        } else {
          this.dataSource.data = this._approvedService.data$.source['_value'].data;
