...
import { PendingService } from 'app/modules/desk1/property/application/transfer/pending/pending.service';
...
// import { Desk1PendingTransferModule } from "./pending.module";
import { FuseConfirmationService } from "@fuse/services/confirmation";

import { cloneDeep } from 'lodash-es';
import { commonViewService } from "app/modules/commonView/commonView.service";
import { CommonViewRoutesModule } from "./commonView.module";
...
export class CommonViewComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<CommonViewRoutesModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<CommonViewRoutesModule>;
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._commonViewService.getAllViewTransferApplications()
      .subscribe(() => {
        if (this._commonViewService.data$.source['_value'].status != 200) {
          // Set the alert
          this.alert = {
            type: 'error',
            message: this._commonViewService.data$.source['_value'].response
          };
        }
          // Show the alert
          this.showAlert = true;
        // } else {
        //   this.dataSource.data = this._commonViewService.data$.source['_value'].data;
...
//     this.__pendingService.approveDesk1(this.approveForm.value)
//         .subscribe((response) => {
...
//                 this.showAlert = true;
//             } else {
//                this._router.navigateByUrl('/desk1/pending-transfer-applications');
...
//   this.__pendingService.rejectDesk1(this.rejectForm.value)
//       .subscribe((response) => {
//           if (response.status != 200) {
//               // Re-enable the form
