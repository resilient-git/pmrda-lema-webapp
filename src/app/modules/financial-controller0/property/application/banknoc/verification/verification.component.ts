...
import { VerificationBankNocService } from './verification.service';
...
import { FinancialControllerVerificationBankNocTransferModule } from "./verification.module";
...
export class FinancialControllerVerificationBankNocComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<FinancialControllerVerificationBankNocTransferModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<FinancialControllerVerificationBankNocTransferModule>;
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._verificationBankNocService.getVerificationFinancialController()
      .subscribe(() => {
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__verificationBankNocService.approveFinancialController(this.approveForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
               ...
               this._router.navigateByUrl('/desk4/verification-transfer-applications');
...
sendRejection(): void {
  ...
  // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__verificationBankNocService.rejectFinancialController(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
