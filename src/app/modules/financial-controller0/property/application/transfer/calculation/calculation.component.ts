...
import { CalculationService } from 'app/modules/financial-controller/property/application/transfer/calculation/calculation.service';
...
import { FinancialControllerCalculationTransferModule } from "./calculation.module";
...
export class CalculationComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<FinancialControllerCalculationTransferModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<FinancialControllerCalculationTransferModule>;
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._calculationService.getCalculationFinancialController()
      .subscribe(() => {
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__calculationService.approveFinancialController(this.approveForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
               ...
               this._router.navigateByUrl('/financial-controller/calculation-transfer-applications');
...
sendRejection(): void {
  ...
  // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__calculationService.rejectFinancialController(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
