...
import { VerificationBankNocService } from 'app/modules/desk4/property/application/banknoc/verification/verification.service';
...
import { Desk4VerificationBankNocTransferModule } from "./verification.module";
...
export class VerificationBankNocComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<Desk4VerificationBankNocTransferModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<Desk4VerificationBankNocTransferModule>;
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._verificationBankNocService.getVerificationDesk4()
      .subscribe(() => {
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__verificationBankNocService.approveDesk4(this.approveForm.value)
        .subscribe((response) => {
...
sendRejection(): void {
  ...
  // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__verificationBankNocService.rejectDesk4(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
