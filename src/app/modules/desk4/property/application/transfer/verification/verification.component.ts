...
import { VerificationService } from 'app/modules/desk4/property/application/transfer/verification/verification.service';
...
import { Desk4VerificationTransferModule } from "./verification.module";
...
export class VerificationComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<Desk4VerificationTransferModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<Desk4VerificationTransferModule>;
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._verificationService.getVerificationDesk4()
      .subscribe(() => {
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__verificationService.approveDesk4(this.approveForm.value)
        .subscribe((response) => {
...
sendRejection(): void {
  ...
  // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__verificationService.rejectDesk4(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
