...
import { VerificationService } from 'app/modules/dy-commissioner/property/application/heirship/verification/verification.service';
...
import { DyCommissionerVerificationHeirshipModule } from "./verification.module";
...
export class VerificationComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<DyCommissionerVerificationHeirshipModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<DyCommissionerVerificationHeirshipModule>;
  ...
  constructor(
    ...
    private __verificationService:VerificationService,
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._verificationService.getVerificationDyCommissioner()
      .subscribe(() => {
        if (this._verificationService.data$.source['_value'].status != 200) {
          // Set the alert
          this.alert = {
            type: 'error',
            message: this._verificationService.data$.source['_value'].response
          };

          // Show the alert
          this.showAlert = true;
        } else {
          this.dataSource.data = this._verificationService.data$.source['_value'].data;
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyHeirshipForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__verificationService.approveDyCommissioner(this.approveForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
               ...
               this._router.navigateByUrl('/dy-commissioner/verification-heirship-applications');
...
sendRejection(): void {
  ...
  // this.propertyHeirshipForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__verificationService.rejectDyCommissioner(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
