...
import { CertificateService } from 'app/modules/dy-commissioner/property/application/heirship/certificate/certificate.service';
...
import { DyCommissionerCertificateHeirshipModule } from "./certificate.module";
...
export class CertificateComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<DyCommissionerCertificateHeirshipModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<DyCommissionerCertificateHeirshipModule>;
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._certificateService.getCertificateDyCommissioner()
      .subscribe(() => {
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyHeirshipForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__certificateService.approveDyCommissioner(this.approveForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
               ...
               this._router.navigateByUrl('/dy-commissioner/certificate-heirship-applications');
...
sendRejection(): void {
  ...
  // this.propertyHeirshipForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__certificateService.rejectDyCommissioner(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
