...
import { AppointmentService } from 'app/modules/dy-commissioner/property/application/banknoc/transfer/appointment/appointment.service';
...
import { DyCommissionerAppointmentTransferModule } from "./appointment.module";
...
export class AppointmentComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<DyCommissionerAppointmentTransferModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<DyCommissionerAppointmentTransferModule>;
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._appointmentService.getAppointmentDyCommissioner()
      .subscribe(() => {
        if (this._appointmentService.data$.source['_value'].status != 200) {
          // Set the alert
          this.alert = {
            type: 'error',
            message: this._appointmentService.data$.source['_value'].response
          };

          // Show the alert
          this.showAlert = true;
        } else {
          this.dataSource.data = this._appointmentService.data$.source['_value'].data;
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__appointmentService.approveDyCommissioner(this.approveForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
               ...
               this._router.navigateByUrl('/dy-commissioner/appointment-transfer-applications');
...
sendRejection(): void {
  ...
  // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__appointmentService.rejectDyCommissioner(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
    ...
    */  
    ...
    //     this._appointmentService.deleteProperty({ code, user_id })
    //       .subscribe((response) => {
    //         if (response.status != 200) {
    //           // Set the alert
    //           this.alert = {
