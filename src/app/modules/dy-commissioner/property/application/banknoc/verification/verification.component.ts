...
import { VerificationBankNocService } from 'app/modules/dy-commissioner/property/application/banknoc/verification/verification.service';
...
import { DyCommissionerVerificationBankNocTransferModule } from "./verification.module";
...
export class VerificationBankNocComponent implements OnInit, OnDestroy {
  ...
  dataSource: MatTableDataSource<DyCommissionerVerificationBankNocTransferModule> = new MatTableDataSource([]);

  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<DyCommissionerVerificationBankNocTransferModule>;
  ...
  constructor(
    ...
    private __verificationBankNocService:VerificationBankNocService,
    private _verificationBankNocService: VerificationBankNocService,
  ...
  this.rejectForm = this._formBuilder.group({
    ...
    this._verificationBankNocService.getVerificationDyCommissioner()
      .subscribe(() => {
        if (this._verificationBankNocService.data$.source['_value'].status != 200) {
          // Set the alert
          this.alert = {
            type: 'error',
            message: this._verificationBankNocService.data$.source['_value'].response
          };

          // Show the alert
          this.showAlert = true;
        } else {
          this.dataSource.data = this._verificationBankNocService.data$.source['_value'].data;
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__verificationBankNocService.approveDyCommissioner(this.approveForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
               ...
               this._router.navigateByUrl('/dy-commissioner/verification-transfer-applications');
...
sendRejection(): void {
  ...
  // this.propertyTransferForm.value.user_property_code = this._activatedRoute.params['_value'].code;
    ...
    this.__verificationBankNocService.rejectDyCommissioner(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
    ...
    */  
    ...
    //     this._verificationService.deleteProperty({ code, user_id })
    //       .subscribe((response) => {
    //         if (response.status != 200) {
    //           // Set the alert
    //           this.alert = {
