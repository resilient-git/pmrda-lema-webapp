...
import { ViewBankNocTransferActionService } from 'app/modules/dy-commissioner/property/application/banknoc/verification/details/action/action.service';
import { MastersService } from 'app/modules/admin/masters/masters.service';
import { FuseAlertType } from '@fuse/components/alert';
import { MatDialog } from '@angular/material/dialog';
import { MatTable, MatTableDataSource } from "@angular/material/table";
import{ ViewBankNocTransferActionModule } from 'app/modules/dy-commissioner/property/application/banknoc/verification/details/action/action.module'
@Component({
    selector       : 'ViewBankNocTransferAction',
...
export class ViewBankNocTransferActionComponent implements OnInit
{
    ...
    pta : any;
    dataSource: MatTableDataSource<ViewBankNocTransferActionModule> = new MatTableDataSource([]);
    displayedColumns: string[] = ['srno','purchaser_name','actions'];
    /**
     * Constructor
     */
    constructor(
        private _viewBankNocTransferActionService: ViewBankNocTransferActionService,
  ...
  */
  ...
  if(this.approveForm && this.approveForm.value){
   ...
   // this.BankNocTransferForm.value.user_BankNoc_code = this._activatedRoute.params['_value'].code;
    ...
    this._viewBankNocTransferActionService.verifyDyCommissioner(this.approveForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
               ...
               this._router.navigateByUrl('/dy-commissioner/banknoc-applications/verification-pending');
             ...
             /// this._router.navigateByUrl('/citizens/transfer-application/fees/'+this.BankNocTransferForm.value['user_BankNoc_code']+'/'+response.transfer_application_id);
            }
          },(response)=>{}
          );
        }
...
sendRejection(): void {
  ...
  // this.BankNocTransferForm.value.user_BankNoc_code = this._activatedRoute.params['_value'].code;
    ...
    this._viewBankNocTransferActionService.rejectDyCommissioner(this.rejectForm.value)
        .subscribe((response) => {
            if (response.status != 200) {
                // Re-enable the form
